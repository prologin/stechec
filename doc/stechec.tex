...
et à \ac{tbt} (ou les sujets prologin) comme la
carrosserie\footnote{Dans ce cas, on peut considérer qu'il fait aussi
  ouvre-vitre électrique}. Les deux parties sont complémentaires, et
devront s'assembler tant et bien que mal.
...
Chaque année, le serveur de la finale prologin \cite{prologin.www} est
refait. Un effort a été fait en 2004 pour faire un programme
suffisamment générique, pour ne pas avoir a tout refaire chaque
année. Nom de code: \emph{Absolut}. Ce fut un quasi-échec pour un tas
de raisons, ce qui a fait qu'il n'a pas pu être repris l'année
suivante.

En 2005, un groupe de personnes motivés\footnote{Certaines pour les
  notes, d'autres pour des raisons plus obscures...} a repris les
choses en main, et tracé le contour d'un nouveau \emph{serveur
  générique}. Petit nom pour ce bébé: \emph{\stechec}\footnote{A la
  base, ce devait être un véritable échec.}. Finalement, quasiment
tout a fonctionné au dela des espérances, mais le code de base n'était
pas assez souple pour pouvoir faire des choses un peu différentes des
sujets prologin.
...
principes furent repris, mais le code fut faconné de manière un peu
...
ils furent mergés\footnote{A l'arrach', comme d'habitude}, avant la
