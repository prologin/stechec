...
référence actuelle est JavaBB \cite{javabb.www}, utilisé par tous les
joueurs invertébrés de Blood Blowl, mais souffre de quelques
défauts. Le graphisme est assez austère, et les versions actuelles ne
sont pas \emph{open source}, ce qui rend plus difficile l'ajout
d'extensions\footnote{Un problème majeur: le développeur, SkyJunkie,
ne semble pas souhaiter pas intégrer les règles du LRB5, ce qui va
surement fruster beaucoup de joueurs.}. L'objectif\footnote{Ce que
l'on espère, il n'est pas dit que ca marchera :)} à long terme de
...
courant.}. Ce document aborde principalement le problème sous l'angle
\emph{technique}, celui du développeur. Il n'a pas pour vocation
d'être un manuel utilisateur, et fournit très peu de détails sur les
graphismes, les effets sonores, l'ambiance de jeu, la réponse à la
vie, l'univers et le reste \cite{forty-two}\ldots Tous les
commentaires à propos de ce document sont les bienvenus.

Les auteurs tiennent à remercier tuxrouge, jibone, jgi, poltuiu,
Elpopotam, Cedric, krys, Vanhu, Tupad, trem, darkside, xilandre, mab,
deaf, et tous les oubliés, pour toutes les remarques pertinentes et
...
L'ancienne version de \ac{tbt}, la version 0.5\footnote{Toujours
disponible en téléchargement sur le site de \ac{tbt} \cite{tbt.www}.},
est vraiment bien coté utilisateur, un peu moins niveau code. La
structure, beaucoup trop monolithique, ajouté à un manque de
documentation chronique, a pas mal freiné le développement, de sorte
qu'il était quasi-impossible d'ajouter un support réseau. L'objectif,
dans un premier temps, est d'obtenir quelque chose de similaire à la
0.5 au niveau du jeu et de l'interface graphique, mais en préparant le
code un peu mieux\footnote{Et avec de la documentation\ldots} pour
pouvoir aller plus loin ensuite.

Voici un résumé des fonctionnalités et grandes lignes que nous nous
sommes efforcé de garder en tête tout en préparant le projet:
...
\item \textbf{Structuration par modules}. Il devrait être ainsi
  ...
  remplacer un coach humain par un coach IA. De plus, plus tard, des
  parties pourront être recodées sans tout refaire, et des modules
  annexes funky pourront être ajoutés.

\item \textbf{Réseau}. Gros manque de la 0.5, c'est pourtant un
  élément essentiel selon nos objectifs. Il est ici prévu dès le
  départ.

\item \textbf{Portabilité} Unix/Windows. Les choix initiaux (C++/SDL)
  nous permettent d'assurer une sortie sur ces deux OS, et nous
  continuerons à nous efforcer à utiliser des bibliothèques qui
  soient portable sur le maximum de système d'exploitation.

\item \textbf{Sécurité}. Un problème des jeux libres est que chacun
  peut modifier le code pour s'octroyer quelques pouvoirs
  supplémentaire. On va s'efforcer de minimiser cet effet en déplaçant
  toute la logique coté serveur, notamment lors de matchs joué en
  ligue (plus de détails dans la partie \ref{sec:securite}).
\end{itemize}

Le jeu de règle retenu est le LRB 5.0 (Living RuleBook), disponible en
français (attention, la traduction peut ne pas être totalement fidèle)
et en anglais sur le site. Dans le futur, il n'est pas exclu que
plusieurs jeux de règles soient supportés, même si aucune méthode pour
cela n'a encore été retenue de manière définitive (voir la partie
\ref{chap:further_idea}).

\section{Licence}

Le code est et restera en source libre, c'est un point non
négociable. Un serveur subversion est accessible en lecture pour tous
sur le site de \ac{tbt}. La licence choisie est la GNU GPL v2. Il est
possible qu'elle puisse être changée par une de ses consoeurs, si il y
en a vraiment besoin. Mais les choses ne sont pas si simples~: peut on
vraiment faire l'adaptation en \emph{open source} d'un jeu qui n'est
pas libre de droits ?

\ac{gw}, contactés à l'époque des premières version de \ac{tbt} avait
autorisé la poursuite du projet à la condition qu'on arrête tout
développement s'ils nous le demandent. Pour jouer à \ac{tbt}
"légalement", il faudrait que chaque joueur posséde le jeu de
plateau.  Nous essayons de reprendre le moins possibles d'éléments
...
voudrais pas écrire trop de bêtises, donc, à voir\ldots
...
\emph{network mode}. Il y aura toujours trois processus (programmes)
lancés: un serveur, et deux clients. Même si le joueur a décidé
d'héberger une partie, un serveur sera lancé sur sa machine et recevra
les connexions. Même si il a décidé de jouer seul sur sa machine
contre une AI, un serveur et un autre client seront lancés dans son
dos depuis l'interface graphique. Aussi bizarre que cela puisse
paraître, ce cas d'utilisation \emph{solo} rentre dans la catégorie
\emph{mode réseau}.
...
\emph{mode standalone}, et ce cas sera son unique cas
d'utilisation. Ce cas nécessite pas mal de travail supplémentaire sur
l'interface graphique, et ne devrait pas être immédiatement
disponible.
...
un ensemble de composants (ou modules, ou paquets, dans ce contexte,
ces trois terminologies sont identiques). Ceux-ci vivent dans des
répertoires source différents, sont compilés en bibliothèque statique
ou dynamique et assemblés selon les besoins. Principalement, il
existe:
...
confusion. Référez vous au glossaire (annexe \ref{chap:glossaire})
pour plus de détails.

En mode de fonctionnement \emph{client/serveur}, il sera possible de
lier le serveur à un gestionnaire de ligue, comme décrit dans la
partie \ref{chap:ligue}.

\section{Emboitement des modules}

La manière dont les modules s'assemblent et intéragissent, aussi bien
à la compilation qu'à l'exécution, est une chose
importante\footnote{Et pas simple. il arrive aussi très souvent que je
m'y perde moi-même...}. A la base, n'oubliez pas qu'il y a deux
morceaux principaux~: le client et le serveur, accompagnés d'un petit
attirail (réseau, outils). Ensuite, le client est découpé en deux
parties, une qui s'occupe du rendu à l'écran et l'autre qui gère les
règles du jeu. De même, le serveur est découpé en deux parties, une
qui gère les connexions réseau et l'autre qui s'occupe des règles du
jeu. Ces deux morceaux (client et serveur) sont également liés au
module transfert de données pour qu'ils puissent communiquer entre
eux.

En plus de ce découpages en entités logique, le code est aussi séparé
en \emph{couches} (ce qui donne une sorte de
\emph{stratification}). Par exemple, une partie du code concernant la
gestion des règles côté client et serveur est identique\footnote{La
duplication de code est quelque chose qui a été évitée au maximum. Dès
qu'il est possible, le code commun (et qui pourrait resservir
ailleurs) est rassemblé dans une bibliothèque, ce qui explique cette
stratification (parfois, c'est trop. J'en conviens).}, et est
rassemblée dans le paquet \emph{Règles de base}.

\begin{figure}[ht!]
  \centering
  \includegraphics[width=13cm]{vue-statique}
  \caption{Vue statique des paquets, dépendances à la compilation.}
  \label{fig:static-view}
\end{figure}

La figure \ref{fig:static-view} résume cet assemblage. Le fait que le
module soit inclus dans \texttt{stechec}\footnote{C'est du code non
spécifique à TBT, qui me sert dans d'autres projets. Je mutualise...}
indique que le code source se trouve dans le répertoire
\texttt{stechec/}. De même, les modules inclus dans \texttt{TBT} se
trouvent dans le répertoire \texttt{bb5}. Pour le module client, c'est
un peu plus compliqué car il peut prendre plusieurs formes, le code
est réparti un peu partout. Cette partie est décrite plus en détail
dans la partie \ref{chap:client}.

A l'exécution, la disposition des modules est montré dans la figure
\ref{fig:dynamic-view}. Même remarque qu'avant, le client, qui peut
revêtir plusieurs formes, sera vu plus en détails. Les communications
entre les modules à l'intérieur se font avec de bêtes appels de
fonctions (pas d'IPC ou autre bizarrerie du genre), suivant une
interface bien définie. Mais actuellement, les interfaces ne sont pas
vraiment bien définies, ce qui rend la chose plus compliquée (ça ne
demande qu'à être corrigé).

\begin{figure}[ht!]
  \centering
  \includegraphics[width=13cm]{vue-dynamique}
  \caption{Vue dynamique des paquets, à l'execution. Les notes sont en
  jaune.}
  \label{fig:dynamic-view}
\end{figure}
...
(description des parties serveur et client) en prenant de l'avance et
en décrivant l'utilisation des paquets ainsi que le protocole de
communication, bien que ces deux éléments ne fassent pas à proprement
parler partie du module transfert de données.

Ce module est codé en C++ et utilise TCP/IP (d'autres extensions,
comme le support http/https, proxy sont envisageable), et possède
aussi une implémentation bien \emph{kludgy} permettant la transmission
de paquets dans la configuration \emph{standalone} (on pourrait aussi
ouvrir des sockets sur localhost, ca reviendrait au même).

\begin{figure}[ht]
  \centering
  \includegraphics[width=8cm]{uml-reseau}
  \caption{Hiérarchie du réseau. (note: LocalCx == DirectCx)}
...
l'objet \texttt{Packet}. Il sera transféré tel quel, en mode binaire,
en prenant bien garde à l'\emph{endianess} de la machine.
...
\item \texttt{flags}: Signale des différences de traitement pour le paquet en cours. A implémenter.
...
d'entier, et utiliser des méthodes pour faire la conversion. C'est pas
très beau, mais ça n'a pas été prévu pour ça, et en pratique il y a
peu de chaînes de caractères à faire transiter.
...
\begin{figure}[th]
  ...
  \begin{tabular}{|l|c|c|l|}
    ...
    \textsc{CX\_QUERYLIST} & X & & Le client demande la liste des parties \\ \hline
    \textsc{CX\_LIST} & & X & Le serveur envoie la liste des parties disponibles \\ \hline
    \textsc{CX\_JOIN} & X & & Le client veut rejoindre une partie \\ \hline
    \textsc{CX\_READY} & X & & Le spectateur est prêt à passer au tour suivant \\ \hline
    \textsc{CX\_ABORT} & X & & Le client veut quitter la partie \\ \hline
    \textsc{MSG\_SYNC} & & X & Un traitement est terminé (a supprimer) \\ \hline
    \textsc{GAME\_FINISHED} & & X & La partie est terminée \\ \hline
    \textsc{CLIENT\_UID} & & X & Un identifiant unique est envoyé au client \\ \hline
  \end{tabular}
  \caption{Liste des tokens de synchronisation de partie. Les colonnes
    \texttt{Cl.} et \texttt{Srv.} indiquent respectivement que le
    token peut être envoyé d'un client ou du serveur.}
  \label{tab:sys-tokens}
\end{figure}

Le tableau \ref{tab:sys-tokens} référence la liste des messages dit
\emph{système}, pour contrôler la connection entre les clients et le
serveur. Ces messages font partie intégrante de \texttt{stechec},
également utilisés pour d'autres programmes que \ac{tbt}. Voici un
exemple typique de communication lors d'une nouvelle connection au
serveur, et du choix d'une partie~:
...
CX_ACCEPT           ->
                    <-       CX_QUERYLIST
CX_LIST             ->
                    <-       CX_JOIN (avec uid de partie)
CX_ACCEPT           ->
CLIENT_UID          ->
\end{lstlisting}

\texttt{CX\_INIT} est envoyé par le client pour initier une connexion,
et contient quelques informations pour l'intégrité (numéro de version,
règles utilisés). Le serveur peut soit accepter cette connection avec
\texttt{CX\_ACCEPT}, ou la refuser avec \texttt{CX\_DENY}. Une fois
ainsi connecté, le client peut lister les parties en cours sur le
serveur (enfin, c'est pas encore codé), puis envoyer un message
\texttt{CX\_JOIN} avec l'identifiant de la partie que le client
souhaite créer ou rejoindre, ainsi que d'autres informations
(spectateur ou acteur, le nom du coach, \ldots). Le serveur pourra
alors lui renvoyer \texttt{CX\_ACCEPT}, suivi d'un
\texttt{CLIENT\_UID} contenant l'identifiant unique que le coach s'est
vu attribué, ou bien \texttt{CX\_DENY} si la demande est refusée.
...
\begin{figure}[th]
  ...
  \begin{tabular}{|l|c|c|l|}
    ...
    Token & Cl. & Srv. & Description \\ \hline
    \hline
    \textsc{MSG\_INITGAME} & X & X & Passe en phase d'initialisation de partie \\ \hline
    \textsc{MSG\_INITHALF} & X & X & Passe en phase d'initialisation de mi-temps \\ \hline
    \textsc{MSG\_INITKICKOFF} & X & X & Phase de placement de la balle sur le terrain\\ \hline
    \textsc{MSG\_BALLPOS} & & X & Donne la position de la balle \\ \hline
    \textsc{ACT\_ILLEGALPROC} & X & & 
       Demande d'interruption provenant de l'adversaire \\ \hline
    \textsc{MSG\_TIMEEXCEEDED} & & X &
       Dépassement du temps d'un tour de jeu \\ \hline
    \textsc{ACT\_DECLARE} & X & & Déclaration d'une action \\ \hline
    \textsc{ACT\_MOVE} & X & & Un joueur bouge \\ \hline
    \textsc{ACT\_STANDUP} & X & & Un joueur se relève \\ \hline
    \textsc{MSG\_CHAT} & X & X & Bavardage en ligne \\ \hline
  \end{tabular}
  \caption{Liste (non exhaustive) des tokens en cours de jeu. Les colonnes
    \texttt{Cl.} et \texttt{Srv.} indiquent respectivement que le
    token peut être envoyé d'un client ou du serveur.}
  \label{tab:game-tokens}
\end{figure}

Le tableau \ref{tab:game-tokens} référence quelque un des types de
messages spécifiques à \ac{tbt}, et qui peuvent être envoyés de part
et d'autre au cours de la partie. Pour voir la liste complète, ainsi
que les informations additionnelles contenues dans ces messages, il
faut se reporter au code source. Une petite aide~: les types de
messages se trouvent dans le fichier
\texttt{tbt/bb5/common/Constants.hh}, et le contenu de ses messages se
trouvent dans les fichiers du même répertoire.


Maintenant, passons à quelques considérations technico-philosophique
sur le réseau et sont influence dans le jeu. Provenant d'un monde où
les gens se voient en vrai (référence au jeu de plateau), et bien que
ce soit un jeu tour à tour, les concepteurs de Blood Bowl ont
introduit la faculté au coach qui glande la possibilité d'interrompre
le tour de l'autre ou de devoir lancer des dés, histoire de ne pas
l'endormir. Par exemple, ca peut se produire lors de l'oubli du
déplacement du marqueur de tour, quand le second coach essaie de
bloquer un joueur plus fort que lui, lors de certaines cartes
spéciales, les sorciers, \ldots Ces possibilités devront être
reproduites.

Cela nous a obligé à orienter le protocole en traitement par
...
client dans l'expectative. 
\begin{itemize}
\item On suppose le protocole comme sûr (pas de
paquet perdu), ce qui est heureusement le cas avec TCP. 
\item On suppose également que les paquets arrivent dans l'ordre. Si
le serveur envoie 2 messages, A et B, le client recevra A puis B et
jamais l'inverse (ce qui est toujours le cas avec TCP).
\end{itemize}
La manière que nous avons d'écrire les règles de jeu repose sur ces
deux règles, si il vient a quelqu'un l'idée d'utiliser un autre
protocole de transport, il devra s'assurer qu'elles seront toujours
respectées.
...
l'interruption d'un autre coach. Les temps de calcul du serveur et les
temps de transfert réseau ne seront pas pris en compte (cad pas
décompté des 4 minutes), parce qu'ils devraient toujours être
inférieur à 0.2 secondes pour une action, temps pendant lequel le
coach peut réfléchir.
...
l'utilisation mémoire, ainsi que de l'ajout d'un schéma de validation
pour permettre la vérification des documents ainsi que de fournir des
valeurs par défaut dans le cas où tout l'utilisateur aurait omis
quelques champs. Et, bien entendu, le
débugger\footnote{Non... vraiment ? :)}.



%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% MODULE REGLES (CLIENT/SERVEUR)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\section{La sécurité, où ¨t'es gentil, H4k3rZ, va jouer ailleurs¨}
\label{sec:securite}
...
En gros, les clients qui viennent de se connecter tombent dans un pool
d'attente, puis rejoignent une partie quand ils le demandent. Ensuite,
a la fin de ladite partie, ils sont purement et simplement déconnecté.
Il n'y a pas grand chose a dire de plus, il suffit de regarder le code
dans \texttt{stechec/server/}
...
Il faut savoir que cette partie est globalement codé, tous les
messages peuvent être enregistré dans un fichier coté serveur, mais la
lecture de ces logs est encore problématique... En fait, il manque un
champ \texttt{timestamp} dans les messages, ce qui fait qu'il n'est
pas possible de prévoir a quel rythme sortir les messages (ce qui est
vraiment problématique, vu que tout le système est asynchrone). A
réfléchir.
...
\begin{wrapfigure}{r}{9cm}
  \includegraphics[width=8.5cm]{package-client}
  \caption{Schéma simplifié de l'environnement du client.}
  \label{fig:package-client}
\end{wrapfigure}

On a vu (section \ref{chap:regles}) le fonctionnement des règles côté
client. Il reste la plus grosse partie, le client. La figure
\ref{fig:package-client} montre les interactions de ce client (la
grosse boîte colorée) avec les règles et la connection serveur (classe
\texttt{ClientCx}). Cette partie est modulaire, on peut mettre
n'importe quoi dans la boîte client (un client graphique, en mode
console, ou sans interaction, c'est à dire une IA). Le client contient
la fonction \texttt{main()}, il est donc responsable d'initialiser les
règles et la connection (mais ca se fait en quelques lignes, via les
autres modules), puis il lui reste à gérer l'affichage et les entrées
utilisateur (clavier, souris), et jouer via la classe \texttt{Api}.

\section{Client générique (chargeur de client)}

En réalité, ce n'est pas un vrai client, c'est une sorte de
\emph{wrapper} qui va s'occuper du sale boulot, c'est à dire de parser
le XML, de se connecter au serveur, de charger les règles, puis de
charger et de donner la main à un vrai client. A l'origine, il a été
conçu pour le concours prologin, pour masquer ces détails aux
candidats, et évitent qu'ils n'y touchent. Puis, comme c'est un outil
bien pratique, je m'en suis reservi pour le client console et
graphique de tbt.

\begin{figure}[ht]
  \centering
  \includegraphics[width=13cm]{client-generique}
  \caption{Fonctionnement du client générique}
  \label{fig:client-generique}
\end{figure}

Le fonctionnement est plutôt simple, et tient dans une fonction
\texttt{main} de 120 lignes. Il est décrit dans la figure
\ref{fig:client-generique}. Les sources se trouvent dans le répertoire
\texttt{stechec/client/}. Après avoir chargé le fichier XML (donné en
ligne de commande, ou \emph{deviné} par le programme en cherchant dans
quelques endroits prédéfinis), il va charger le module de règles
client défini dans le fichier de configuration XML, puis appeler cette
fonction qui doit être codé dans les règles:

\begin{lstlisting}[language=C++]
extern "C" BaseCRules* load_client_rules(xml::XMLConfig* cfg);
\end{lstlisting}

Ensuite, après avoir chargé le module de connection, il peut se
connecter directement au serveur, si il y a
\texttt{auto\_connect=true} dans le XML. Le nom et le port du serveur
sont également dans le XML (aucune interaction utilisateur à ce
niveau). Après s'être connecté au serveur, il appelle cette fonction
si elle est présente dans les règles (TBT ne l'utilise pas, mais
c'était nécessaire pour prologin, pour que les règles clients puissent
être initialisé par le serveur avant de charger le code des candidats):

\begin{lstlisting}[language=C++]
extern "C" bool hook_before_ui_load(ClientCx* ccx, BaseCRules* rules);
\end{lstlisting}

Il est a noter que le module CCX est l'abbréviation de \emph{Client
  Connexion}. Ce module fait pour l'instant partie intégrante du
client générique, mais devrait être bougé dans une bibliothèque
séparée. Ce client générique est bien pratique dans un certain nombre
de cas, mais quand on veut faire quelque chose de plus compliqué (par
exemple, une interface graphique avec un menu dans lequel il est
possible de choisir ses paramètres de connections au serveur, de se
connecter/déconnecter plusieurs fois sans quitter l'application), il
vaut mieux ne pas l'utiliser et le recoder dans le client en question.
...
éléments internes de l'IA sont laissés à l'entière responsabilité de
leur concepteur\footnote{Autrement dit: démerdez vous pour faire
  quelque chose de potable :).}.

\section{Client GUI/SDL 2D}
...
\subsubsection{Les ressources SDL}
...
\subsubsection{La classe Surface et dérivés}
...
\texttt{TextSurface} est pratique pour afficher du texte, via sa
méthode setText()...

\subsubsection{La classe VirtualSurface, et le rendu}

C'est la seconde classe la plus importante derrière
\texttt{Surface}. Elle peut être vue comme une sorte de
...
recursivement depuis la surface parent, l'écran, puis render).

\subsubsection{VirtualScrollableSurface}

Cette classe est un raffinement de \texttt{VirtualSurface} pour
permettre la création d'une surface \emph{virtuelle} qui sera plus
grande que ce qui sera affiché réellement à l'écran. Elle est utilisée
pour l'affichage du champ de bataille, qui est plus grand que ce qu'on
peut afficher à l'écran. Cette classe s'occupe du déplacement du
\emph{champ de vision}. En fait, c'est comme si c'était une
\texttt{VirtualSurface}, avec quelques ennuis du coté de la
récupération des entrées utilisateur (la souris).

\begin{figure}[ht]
  \centering
  \includegraphics[width=14cm]{gui-vscrollable}
  \caption{Détail du fonctionnement d'un objet \texttt{VirtualScrollableSurface}}
  \label{fig:gui-vscrollable}
\end{figure}

La figure \ref{fig:gui-vscrollable} montre plus en détail le
fonctionnement d'un objet de type \texttt{VirtualScrollableSurface},
en prenant pour exemple le terrain de jeu de bloodbowl.

Il faut bien faire attention au référentiel dans lequel on
travaille. La plupart du temps, pour placer/déplacer les joueurs, la
balle sur le terrain, on utilise celui retourné par \texttt{getRect()}
(et les autres méthodes qui vont avec), comme si on travaillait sur
une \texttt{VirtualSurface} classique. Dans ce cas, on se fiche bien
de savoir ce que vois réellement le coach, on fait comme si toute la
surface sera affichée.

Par contre, il arrive qu'on ait besoin de savoir ce que voit
réellement le coach (où est le champ de vision), principalement pour
gérer les entrées utilisateur. Dans ce cas, on va plutôt utiliser la
méthode \texttt{getRealRect()} (ou mieux,
\texttt{getRealAbsoluteRect()}, puisque dans ce cas on veut les
coordonnées par rapport à l'origine de la fenêtre, pas juste de la
surface virtuelle). Cette méthode peut nous retourner une surface en
dehors du champ de vision, dans ce cas, l'objet n'est pas visible.

On a pas souvent besoin de passer de l'un a l'autre de ces
référentiel. Pour l'affichage, on utilisera toujours le premier
référentiel, et l'object \texttt{VirtualScrollableSurface} s'occupera
d'afficher la zone qui va bien. Si on veut un objet \emph{flottant} au
dessus du terrain, qui reste toujours à la même place à l'écran
quelque soit la position du champ de vision, il vaut mieux l'ajouter
au parent de \texttt{VirtualScrollableSurface}, avec un Z plus grand.
Le seul cas où la conversion est nécessaire est pour récupérer la
position sur le terrain depuis d'un clic ou déplacement de la
souris. Dans ce cas il suffit d'additionner les 2 coordonnées
\texttt{x, y} des 2 référentiels.

\subsubsection{Références, mémoire}
...
De manière générale, on va essayer de stocker le moins d'informations
d'états possible, en se basant sur les différents autres modules. Un
code sans état est toujours moins buggé qu'un qui essaie d'en avoir
trop. L'interface graphique se trouve entre deux fronts:

\begin{itemize}
\item \texttt{Les règles du jeu (API)}: On essaie d'être le plus
\emph{stateless} possible. C'est à dire qu'on va piocher tout le temps
les informations dans l'API règle. Cela évite d'avoir des informations
redondantes, d'éviter de se retrouver dans un état bancal, et de
débugger plus facilement l'API règle en évitant de contourner les
comportements bizarre dans le code de l'interface graphique. Il y a
certains moments où l'API nous oblige à stocker quelques informations
d'états, comme entre la déclaration et la réalisation d'actions pour
les joueurs. Dans ce cas, petite entorse à la règle, mais croyez le,
c'est en général une belle source de bugs.

\item \texttt{Les entrées utilisateurs et l'affchage}: Pour les
entrées, pas le choix, il faut stocker les différents états, affichage
ou non du menu contextuel, \ldots. En ce qui concerne l'affichage, vu
qu'on se base sur le moteur décrit précédement, il suffit de créer des
objets graphiques, de leur assigner une position, et c'est tout.
...
\begin{verbatim}
   ...
   VisuPlayer     Panel    |             VisuField
                       ActionPopup
          Game                     ActionDlg <---- DialogBox
\end{verbatim}

\texttt{Game} est la pièce centrale du puzzle. Elle contient l'api, la
boucle de jeu principale, la fenêtre SDL, et des instances de toutes
les classes sous-mentionnées. Le plus important, c'est que tous les
autres objets de l'interface graphique contiennent une référence vers
\emph{Game}, et de la peuvent accéder à tous les autres objets. C'est
une sorte de globale.

\texttt{Panel} représente la partie de droite de l'écran. C'est une
des parties les plus simple a comprendre. Elle n'a pas à gérer (pour
l'instant) d'entrée utilisateur. Elle charge pas mal de textures et de
fonts, affiche ce qu'on lui demande via quelques méthodes publique, et
va poller l'API pour afficher le reste (par exemple, le nombre de
rerolls restants).

\texttt{VisuField} représente, sans grande surprise, le terrain de jeu
(la partie de gauche). Elle gère principalement le background et le
ballon. Le ballon est la partie la plus délicate, car elle requiert
pas mal d'interaction avec l'utilisateur lors du kickoff.

\texttt{VisuPlayer} est une des plus importante. Elle représente et
gère un joueur affiché sur le terrain, ainsi que tout ce qui gravite
autour de lui: cercle de sélection, petite icone de status, \ldots. Il
peut donc y avoir au maximum 2 * 11 instances de cette classe, qui
sont gérés directement par \emph{Game}.

\texttt{ActionPopup} est la fenêtre. Elle interagit principalement
avec \emph{VisuPlayer}.

\texttt{DialogBox} est l'implémentation des fenêtres
\emph{popup}. Elle pourrait se retrouver dans le moteur SDL de base,
mais pour l'instant elle ne permet que d'afficher des fenêtres
spécifiques pour \ac{tbt} (exemple le plus flagrant: la fenêtre
présentant les dés de blocage). Il y a un système de callback très
simple, pour savoir sur quel bouton l'utilisateur a appuyé. Il suffit
de créer une instance d'un objet dérivé de \texttt{DialogBoxCb} et de
lui passer, pour qu'au moment opportun (le clic de souris) une méthode
virtuelle soit appelée.

\texttt{ActionDlg} est une classe prévue pour \emph{soulager} la
classe \texttt{Game}. En gros, elle permet d'ajouter, regroupe la
liste des boîtes de dialogue ouverte, et gère les callbacks en les
renvoyant à la bonne personne. Le plus souvent, vers un appel à l'API
des règles.
...
l'opération prenant tout le temps CPU est le blit de surface
SDL. Donc, une fois les blits sont a peu près optimisé (cad, en faire
le moins possible), ca suffit. Ca veut dire que je ne cherche même pas
a optimiser le reste du code, on verra plus tard a coup de callgrind
si il y a des parties de code trop consommatrices.
...
On a déjà décrit 3 clients, dont 2 dont l'implémentation est déjà bien
avancée. On peut imaginer en avoir d'autres, par exemple un affichage
en 3D utilisant un moteur 3D quelconque. Il suffit de se connecter à
l'API et au module de connection client/serveur, et on n'a plus qu'a
se concentrer sur l'affichage. 

D'autres clients pourraient être possible, comme un client permettant
de jouer des parties par mail.
...
le format exact est quasiment fixé (voir les DTD dans le dépôt). Il
sera en XML, et devrait se rapprocher le plus possible de ce
qu'utilise actuellement javaBB.

Il existe maintenant pas loin de trois éditeur d'équipe, 2 en
php/html, accessible et fonctionnelle sur le site, et une codé en C++
utilisant SDL et la bibliothèque Paragui.
...
\item Une documentation décrivant la modélisation générale, décrivant
  ...
  soft. Ce document convient très bien.
...
On est reparti de zéro, mais depuis le début de l'année 2006 (date de
reprise du code), pas mal de boulot a déjà été abattu. L'objectif, je
le rappelle, est une version 0.7 qui ne doit pas être trop inférieure
à l'actuelle 0.5. On devrait retrouver un moteur de règles quasi
complet, au moins jouable, une interface graphique similaire, et le
mode réseau fonctionnel. Le gestionnaire de ligue, l'intelligence
artificielle et l'éditeur d'équipe restent optionels pour la 0.7, mais
si ils sont terminés d'ici là, tant mieux. Tout cela est décrit de
manière plus précise sur le trac, en particulier dans la section
\emph{roadmap}. Les différents modules ainsi que leur état
d'avancement sont également précisé plus en détail sur le wiki.

Si vous êtes interessé pour commencer ou continuer une partie, vous
...
\item Pour décrire les règles coté serveur, on utilise le mot
  \emph{arbitrage}.
\item Paquet, composant, module: ces termes sont un peu utilisés en
vrac, mais veulent souvent dire la même chose.
\end{itemize}
