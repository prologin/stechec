...
template <typename T>
int        Field<T>::getNbTackleZone(int team_id, const Position& pos)
{
  int res = 0;
  Position p;
...
template <typename T>
int Field<T>::getScoreModifier(const ScorePoint& cur,
...
template <typename T>
bool        Field<T>::getMinPath(int team_id)
{
...
template <typename T>
const PosList& Field<T>::getPath(const Position& start,
                                           const Position& dest,
                                           T* p)
{
  // Reinit path_tab_.
