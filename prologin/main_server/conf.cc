...
static void	read_token_conf(const char *line, int num_line, struct s_conf *conf);
...
/*
** Read vonfiguration for "Token" 
*/
static void	read_token_conf(const char *line, int num_line, struct s_conf *conf)
{
  if (strncmp(line, "  end: ", 7) == 0)
    conf->end_cmd = strdup(line + 7);
  else
    fprintf(stderr, "./meta_server: Error in conf file line %d\n", num_line);
}
