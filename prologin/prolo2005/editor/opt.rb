...
class Option
  ...
  def initialize
    ...
    TkcImage.new(canv, 8, 8, :image => @bitmap['water'])
    ...
    but = TkRadioButton.new(@fMap,
                            ...
                            :variable => @pencil)
    canv = TkCanvas.new(@fMap, :borderwidth => 0, :width => 16, :height => 16)
    TkcImage.new(canv, 8, 8, :image => @bitmap['ore'])
    TkGrid.grid(but, canv, :sticky => 'w')
    ...
    but = TkRadioButton.new(@fMap,
                            ...
                            :variable => @pencil)
    canv = TkCanvas.new(@fMap, :borderwidth => 0, :width => 16, :height => 16)
    TkcImage.new(canv, 8, 8, :image => @bitmap['plasma'])
    TkGrid.grid(but, canv, :sticky => 'w')
  ...
  def loadConfig
    ...
    # mode lecture seule
    if $restricted
      [@fMap, @fPlayer, @fExtra].each { |x| x.unpack }
    else
      [@fMap, @fPlayer, @fExtra].each { |x|
        x.pack('side' => 'top',
               :padx => 5,
               :pady => 5,
               :fill => 'x')
