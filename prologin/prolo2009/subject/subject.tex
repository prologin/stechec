...
\section{Introduction}
  ...
  \subsection{Le plateau de jeu}
    ...
    TAILLE\_CARTE * TAILLE\_CARTE cases ce qui revient à un terrain de jeu
  ...
  \subsection{Chacun mon tour}
    Chaque protagoniste jouera l'un après l'autre\footnote{Attention à bien
    vérifier que c'est à votre tour de jouer grâce à la fonction mon\_tour()
    décrite dans l'API, sans quoi vos actions ne seront pas executées}. Dans
    la première phase de jeu, vous pourrez construire des routes et des maisons
    puis, dans une deuxième phase, vous vous affronterez sans merci dans une
    lutte pour la corruption afin jouir du privilège d'agir seul dans la
    troisième et dernière phase.

    \subsubsection{Ordre des joueurs}
      Chaque tour est donc composé de 3 phases. Chacun des joueurs jouera
      successivement la phase I avec les eventuelles constructions et/ou
      destructions faites par les joueurs ayant agit avant lui. Lors de la phase
      II, les participant jouent cette fois de façon simultanée.\\
      À la fin du tour l'ordre des joueurs change. Ainsi celui qui jouait en
      deuxième joue en premier, le troisème devient le deuxème et le premier
      jouera en troisème. %% améliorer ça ?

  \subsection{La concurrence}
    La concurrence est rude dans le domaine, d'autres ont flairés le bon coup.
    Deux autres joueurs\footnote{Dans des conditions rééles de jeu, vous serez
    donc trois, vous inclus} tenterons de vous surpasser. Tout le monde se
    connait dans le domaine et vu votre passé et celui de vos adversaires il est
    évident que vous êtes fichés. Vous ne connaissez d'ailleurs pas leur nom
    mais uniquement leur matricule\footnote{Il s'agit de l'id des joueurs
    renvoyé par les fonctions du jeu}. Le votre est 0 tandis que ceux des deux
    autres \emph{investisseurs} sont les suivants entre 1 et le nombre de
    joueurs - 1.
...
\section{Phase II} % Corruption, système d'enchère
  ...
  \subsection{Les bâtiments exceptionnels}
    ...
    En cas d'égalité, celui qui jouera en dernier (parmis ceux en égalité) la
    phase I au prochain tour remporte l'enchère.
...
\section{Phase III} % Placement des monuments, valeur des terrains
  ...
  \subsection{Liste des bâtiments}
    ...
    \begin{itemize}
      \item \textbf{Préstige Portée Nom}. Description.
	...
	parc d'attractions est fait pour vous ! Venez découvrir les nombreuses
      ...
      \item \textbf{+4 1 Aquarium}. Qui n'a jamais révé de nager avec les
	dauphins ? de prendre une bonne dose d'adrénaline avec les requins ? Le
	parc aquatique vous permettra d'effleurer votre rêve et de comtempler
	...
	incoutournable pour son paysage sans cesse renouvelé, ses riches
	...
	grands !
      ...
      en exclusivité, \textit{Platonov} d'Anton Tchékhov interprété par la
	...
	spectacles musicaux tous les samedi soirs. À ne surtout pas manquer !

      \item \textbf{+2 1 Supermarché}. Vous y trouverez tout ce dont vous pouvez
	avoir besoin au quotidien  mais aussi les spécialités culinaires
	locales\up{*}\\
	...
	rude journée de travail.
      ...
      \item \textbf{+1 3 Centre commercial}. Un lieu indispensable pour tout fan
	de shopping qui se respecte. Vous y trouverez les vêtements à la mode
	...
	visiter gratuitement les lieus. Étrangement, ce lieu ne fait pas
