...
{
  fprintf (stderr, "usage : meta-server [-cdipv ...]\n");
...
{
  fprintf (stderr, "usage : meta-server [-cdipv ...]\n");
  fprintf (stderr, "Try \'meta-server -h\' for more informations\n");
  exit (1);
}
...
void	fill_arg(int argc, char **argv, struct s_conf **conf)
{
  ...
  if (!*conf)
    *conf = init_conf();
  while ((ch = getopt_long(argc, argv, "c:d:i:p:v:h", longopts, NULL)) != -1)
    {
      i = is_option(ch);
      if (i == -1 && ch == '?')
	print_usage();
