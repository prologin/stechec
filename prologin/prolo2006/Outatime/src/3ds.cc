...
// Basically, how it works is, you load a chunk, then you check the
// chunk ID. Depending on the chunk ID, you load the information that
// is stored in that chunk. If you do not want to read that
// information, you read past it. You know how many bytes to read past
// the chunk because every chunk stores the length in bytes of that
// chunk.
...
{
  // Open the 3DS file
  if (0 == (m_FilePointer = fopen(strFileName, "rb")))
    return false;
  ...
  // Now we actually start reading in the data.
  // ProcessNextChunk() is recursive

  // Begin loading objects, by calling this recursive function
  ProcessNextChunk(pModel, m_CurrentChunk);

  // After we have read the whole 3DS file, we want to calculate our
  // own vertex normals.
...
{
  t3DObject newObject;			// Used to add to our object list
  tMaterialInfo newTexture;		// Used to add to our material list
  unsigned int version = 0;		// To hold the file version
  int buffer[50000] = {0};		// Used to read past unwanted data

  m_CurrentChunk = new tChunk;

  // Below we check our chunk ID each time we read a new chunk.  Then,
  // if we want to extract the information from that chunk, we do so.
  // If we don't want a chunk, we just read past it.

  // Continue to read the sub chunks until we have reached the length.
  // After we read ANYTHING we add the bytes read to the chunk and
  // then check
  ...
  while (pPreviousChunk->bytesRead < pPreviousChunk->length)
	...
	case VERSION:
	  ...
	  // Read the file version and add the bytes read to our
	  // bytesRead variable
	  m_CurrentChunk->bytesRead += fread(&version, 1,
					     m_CurrentChunk->length -
					     m_CurrentChunk->bytesRead,
					     m_FilePointer);

#warning "FIXME: version du fichier 3DS"
// 	  if (version > 0x03)
// 	    erreur;
	  break;
	...
	case OBJECTINFO:
	  ...
	  m_TempChunk->bytesRead += fread(&version, 1,
					  m_TempChunk->length -
					  m_TempChunk->bytesRead,
					  m_FilePointer);
	  m_CurrentChunk->bytesRead += m_TempChunk->bytesRead;

	  // Go to the next chunk, which is the object has a texture,
	  // it should be MATERIAL, then OBJECT.
	  ProcessNextChunk(pModel, m_CurrentChunk);
	  break;
	...
	case MATERIAL:
	  ...
	  // Add a empty texture structure to our texture list.  If
	  // you are unfamiliar with STL's "vector" class, all
	  // push_back() does is add a new node onto the list.  I used
	  // the vector class so I didn't need to write my own link
	  // list functions.
	...
	case OBJECT:
	  // This chunk is the header for the object info chunks.  It
	  // also holds the name of the object.
	  ...
	  memset(&(pModel->pObject[pModel->numOfObjects - 1]), 0,
		 sizeof(t3DObject));

	  // Get the name of the object and store it, then add the
	  // read bytes to our byte counter.
	  m_CurrentChunk->bytesRead +=
	    GetString(pModel->pObject[pModel->numOfObjects - 1].strName);

	  // Now proceed to read in the rest of the object information
	  ProcessNextObjectChunk(pModel,
				 &(pModel->pObject[pModel->numOfObjects - 1]),
				 m_CurrentChunk);
	  break;

	case EDITKEYFRAME:
	  // Because I wanted to make this a SIMPLE tutorial as
	  // possible, I did not include the key frame information.
	  // This chunk is the header for all the animation info.  In
	  // a later tutorial this will be the subject and explained
	  // thoroughly.
	...
	default:
	  // If we didn't care about a chunk, then we get here.  We
	  // still need to read past the unknown or ignored chunk and
	  // add the bytes read to the byte counter.
	  m_CurrentChunk->bytesRead += fread(buffer, 1,
					     m_CurrentChunk->length -
					     m_CurrentChunk->bytesRead,
					     m_FilePointer);
      ...
      // Add the bytes read from the last chunk to the previous chunk
      // passed in.
...
//
// This function handles all the information about the objects in the
// file
//
						  ...
						  tChunk *pPreviousChunk)
{
  ...
  // Continue to read these chunks until we read the end of this sub
  // chunk
  while (pPreviousChunk->bytesRead < pPreviousChunk->length)
    {
      // Read the next chunk
	...
	{
	case OBJECT_MESH:
	  // We found a new object, so let's read in it's info using
	  // recursion
	...
	case OBJECT_MATERIAL:
	  ...
	  // has assigned to it.  This could either be just a color or
	  // a texture map.  This chunk also holds the faces that the
	  // texture is assigned to (In the case that there is
	  // multiple textures assigned to one object, or it just has
	  // a texture on a part of the object.  Since most of my game
	  // objects just have the texture around the whole object,
	  // and they aren't multitextured, I just want the material
	  // name.

	  // We now will read the name of the material assigned to
	  // this object
...
//
// This function handles all the information about the material
// (Texture)
//
...
void		CLoad3DS::ReadVertexIndices (t3DObject *pObject,
					     tChunk *pPreviousChunk)
{
  unsigned short index = 0;		// Current face index
					     ...
					     tChunk *pPreviousChunk)
{
  ...
  pPreviousChunk->bytesRead += fread(&pObject->numTexVertex, 1, 2,
				     m_FilePointer);
					...
					tChunk *pPreviousChunk)
{
  ...
  pPreviousChunk->bytesRead += fread(&(pObject->numOfVerts), 1, 2,
				     m_FilePointer);

  // Allocate the memory for the verts and initialize the structure
  pObject->pVerts = new Vector3D [pObject->numOfVerts];
  ...
  // Go through all of the vertices that we just read and swap the Y
  // and Z values
...
//
// This function reads in the material name assigned to the object and
// sets the materialID
//
void		CLoad3DS::ReadObjectMaterial (t3DModel *pModel,
					      ...
					      tChunk *pPreviousChunk)
{
  char strMaterial[255] = {0};		// Material name
  int buffer[50000] = {0};		// Used to read past unwanted data
  ...
  for(int i = 0; i < pModel->numOfMaterials; i++)
	...
	{
	  // Set the material ID to the current index 'i' and stop
	  // checking
	  pObject->materialID = i;

	  // Now that we found the material, check if it's a texture
	  // map.  If the strFile has a string length of 1 and over
	  // it's a texture
	  if(strlen(pModel->pMaterials[i].strFile) > 0)
	      // Set the object's flag to say it has a texture map to
	      // bind.
  ...
  pPreviousChunk->bytesRead += fread(buffer, 1,
				     pPreviousChunk->length -
				     pPreviousChunk->bytesRead,
				     m_FilePointer);
}
  ...
  for(int index = 0; index < pModel->numOfObjects; index++)
    {
      ...
      Vector3D * pNormals	= new Vector3D [pObject->numOfFaces];
      Vector3D * pTempNormals	= new Vector3D [pObject->numOfFaces];
      pObject->pNormals		= new Vector3D [pObject->numOfVerts];
      ...
      for(int i = 0; i < pObject->numOfFaces; ++i)
	{
	  ...
	  // Now let's calculate the face normals (Get 2 vectors and
	  // find the cross product of those 2)

	  const Vector3D v1(vPoly[0] - vPoly[2]);
	  const Vector3D v2(vPoly[2] - vPoly[1]);
	  Vector3D Normal(v1 ^ v2);
	  pTempNormals[i] = Normal;	// Save the un-normalized normal
	  Normal.Normalize ();
	  pNormals[i] = Normal;
	}
      ...
      for (int i = 0; i < pObject->numOfVerts; ++i)
	{
	  ...
	  for (int j = 0; j < pObject->numOfFaces; ++j)
		  ...
		  pObject->pFaces[j].vertIndex[2] == i)
		{
		  ...
		  Sum = Sum + pTempNormals[j];
		  // Increase the number of shared triangles
		  shared++;
		}
	    }
	  // Get the normal by dividing the sum by the shared.  We
	  // negate the shared so it has the normals pointing out.
	  pObject->pNormals[i] = Sum / (-shared);
